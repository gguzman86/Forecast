...
library(forecast)
setwd("C:/Users/qproject-hp/Desktop/Proyectos/El Piojo/ANEXO Proyección/ARIMA")
datos <-read.csv("C:/Users/qproject-hp/Desktop/Proyectos/El Piojo/ANEXO Proyección/ARIMA/ARIMATurismo.csv")
dato <-na.omit(datos)

v1 <- ts(dato$Var_Pernoc, start=c(2005), end=c(2015), frequency=1) #Put the corresponding years
par(mfrow=c(2,2))
plot.ts(v1)
...
v1arima <- arima(v1, order=c(0,0,0))  #Put the values of AutoArima 
v1arima
...
v2 <- ts(dato$Var_Pernoc, start=c(2005), end=c(2015), frequency=1) #Put the corresponding years
par(mfrow=c(2,2))
plot.ts(v2)
auto.arima(v2, trace=TRUE) #The thest "adf" relates to Adjusted Dickey Fuller
acf(v2) #Plot correlogram (autocorrelations)
pacf(v2) #Plot partial correlogram
v2arima <- arima(v2, order=c(0,0,0), include.mean = FALSE)  #Put the values of AutoArima 
v2arima
...
v3 <- ts(datos$log_Pernoctacion, start=c(2004), end=c(2015), frequency=1) #Put the corresponding years
par(mfrow=c(2,2))
plot.ts(v3)
auto.arima(v3, test="adf", trace=TRUE) #The thest "adf" relates to Adjusted Dickey Fuller
...
write.csv(resultados, file = "ArimaTurismo.csv")